...
export default function LoginForm() {
  ...
  const handleApiCall = async (userData) => {
    try {
      dispatch(clearError());
      const response = await postRequest("/api/v1/user/login", userData);
      if (response.success) {
        dispatch(login());
    ...
    try {
      ...
      const user = result.user;
        if (!user) {
          dispatch(
            setError(
              "An error occurred while logging up with Google. Please try again"
            )
          );
          return;
        }
      ...
      dispatch(
        setError(
          error.message || "An error occurred while logging up with Google"
        )
      );
    }
  }
  ...
  return (
    <>
      <Card shadow={false}>
        ...
        <CardBody>
          <form onSubmit={handleLogin} className="flex flex-col gap-4 md:mt-12">
            <div>
              <label htmlFor="email">
                <Typography
                  variant="small"
